...
class MusicBot(discord.Client):
    ...
    async def get_player(self, channel, create=False) -> MusicPlayer:
        ...
        if server.id not in self.players:
            ...
            player = MusicPlayer(self, voice_client, playlist) \
                .on('воспроизвести', self.on_player_play) \
    ...
    async def cmd_помощь(self, command=None):
        """
        Usage:
            {command_prefix}помощь [command]
        ...
        if command:
            ...
            for att in dir(self):
                if att.startswith('cmd_') and att != 'cmd_помощь':
    ...
    async def cmd_черныйсписок(self, message, user_mentions, option, something):
        """
        Usage:
            {command_prefix}черныйсписок [ + | - | добавить | удалить ] @UserName [@UserName2 ...]

        Добавление или удаление пользователей из черного списка.
        Пользователи в черном списке не могут пользоваться ботом.
        """

        if not user_mentions:
            raise exceptions.CommandError("Не найдено пользователей.")

        if option not in ['+', '-', 'добавить', 'удалить']:
            raise exceptions.CommandError(
                'Нельзя использовать "%s" , используйте +, -, добавить, или удалить' % option
            )

        for user in user_mentions.copy():
            if user.id == self.config.owner_id:
                print("[Commands:Blacklist] Пользователь больше не в черном списке.")
                user_mentions.remove(user)

        old_len = len(self.blacklist)

        if option in ['+', 'добавить']:
            self.blacklist.update(user.id for user in user_mentions)

            write_file(self.config.blacklist_file, self.blacklist)

            return Response(
                '%s пользователь добавлен в черный список' % (len(self.blacklist) - old_len),
                reply=True
            )

        else:
            if self.blacklist.isdisjoint(user.id for user in user_mentions):
                return Response('Ни один из этих пользователей не внесен в черный список.', reply=True)
                ...
                return Response(
                    '%s пользователь удален из черного списка' % (old_len - len(self.blacklist)),
                    reply=True
                )
    ...
    async def cmd_joinserver(self, message, server_link=None):
        ...
        if self.user.bot:
            ...
            return Response(
                "Ну пойдем!  Нажми здесь, чтобы пригласить меня: \n{}".format(url),
    ...
    async def cmd_воспроизвести(self, player, channel, author, permissions, leftover_args, song_url):
        """
        Usage:
            {command_prefix}воспроизвести ссылка_на_песню
            {command_prefix}воспроизвести текстовый_поиск

        Добавьте песню в плейлист.  Если выбран текстовый поиск,
        то в плейлист будет добавлена первая ссылка с youtube.
        """
        ...
        if permissions.max_songs and player.playlist.count_for_user(author) >= permissions.max_songs:
            raise exceptions.PermissionsError(
                "Добавлено в очередь: (%s)" % permissions.max_songs, expire_in=30
            )
        ...
        if not info:
            raise exceptions.CommandError("Видео не может быть воспроизведено.", expire_in=30)
        ...
        if 'entries' in info:
            ...
            reply_text = "Песня **%s** добавлена в очередь. Позиция: %s"
